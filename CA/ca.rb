...
ac = [
  ...
  [0,2,1,7,0,1,4,0,1,4,2,0,0,0,0,0,0],
  [0,2,0,2,2,2,2,2,2,0,2,0,0,0,0,0,0],
  [0,2,7,2,0,0,0,0,2,1,2,0,0,0,0,0,0],
  [0,2,1,2,0,0,0,0,2,1,2,0,0,0,0,0,0],
  [0,2,0,2,0,0,0,0,2,1,2,0,0,0,0,0,0],
  [0,2,7,2,0,0,0,0,2,1,2,0,0,0,0,0,0],
  [0,2,1,2,2,2,2,2,2,1,2,2,2,2,2,0,0],
  [0,2,0,7,1,0,7,1,0,7,1,1,1,1,1,2,0],
...
def pattan(c, t, r, b, l)
  [
    c*10000 + t*1000 + r*100 + b*10 + l,
    c*10000 + l*1000 + t*100 + r*10 + b,
    c*10000 + b*1000 + l*100 + t*10 + r,
    c*10000 + r*1000 + b*100 + l*10 + t
  ]
end
...
while true do 
  ...
  iMax.downto(0) do |i|
    jMax.downto(0) do |j|
      ...
      # 4種類のパターンを作る 
      acmatch = pattan(c, t, r, b, l)

      # 次のセルの状態を探す
      acmatch.each do |x|
        if hash.key? x
          tmp_ac[i][j] = hash[x] 
          next if tmp_ac[i][j] == 0
          if i == 0
            min_i = true
          elsif j == 0
            min_j = true
  ...
  # [0]を増やす
  if min_i == true
    ac.unshift(Array.new(ac[0].size, 0))
    min_i = false
  end

  # [max]を増やす
  if max_i == true
    ac << Array.new(ac[0].size, 0)
    max_i = false
  end
